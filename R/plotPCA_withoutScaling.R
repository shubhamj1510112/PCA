...
plotpca_withoutscaling <- function(genedata, metadata){
  ...
  print({p2 = fviz_pca_ind(gene_pca, habillage=df$Time,pointsize=3,pointshape = 19,
                           label = "none", legend.title = "Time", invisible="quali", 
                           title="PCA Plot Without Scaling")})
  dev.off()
  if (!require("plotly")) install.packages("plotly")
  require(plotly)
  p2 = fviz_pca_ind(gene_pca, habillage=df$Time,pointsize=3,pointshape = 19,
                    label = "none", legend.title = "Time", invisible="quali", 
                    title="PCA Plot Without Scaling")
